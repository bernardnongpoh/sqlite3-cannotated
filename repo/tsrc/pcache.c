...
// ROR Label 12
if((p->pSynced != pPage) != (p->pSynced == pPage))  {
asm volatile (""::: "memory");
...
// COR Label 21
if((!p->pSynced || 0 == (pPage->flags & 8)) != (!p->pSynced && 0 == (pPage->flags & 8)))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 21);
}

// ROR Label 22
if((0 != (pPage->flags & 8)) != (0 == (pPage->flags & 8)))  {
asm volatile (""::: "memory");
	report_label_coverage("ROR", 22);
}

// MCC Label 23
if(!p->pSynced && 0 == (pPage->flags & 8) ) {
asm volatile (""::: "memory");
...
// ROR Label 36
if((sqlite3Config.pcache2.xInit != 0) != (sqlite3Config.pcache2.xInit == 0))  {
asm volatile (""::: "memory");
...
// ABS Label 37
if(sqlite3Config.pcache2.xShutdown < 0)  {
asm volatile (""::: "memory");
...
// ROR Label 46
if((pCache->eCreate != 2) != (pCache->eCreate == 2))  {
asm volatile (""::: "memory");
...
// COR Label 48
if((pPg || (pPg->nRef || (pPg->flags & 8))) != (pPg && (pPg->nRef || (pPg->flags & 8))))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 48);
}

// MCC Label 49
if(pPg && ( pPg->nRef && ( pPg->flags & 8 ) ) ) {
asm volatile (""::: "memory");
...
for(; pPg && pPg->nRef; pPg=pPg->pDirtyPrev) {;
};
// ABS Label 73
if(pPg < 0)  {
asm volatile (""::: "memory");
...
// COR Label 81
if((rc != 0 || rc != 5) != (rc != 0 && rc != 5))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 81);
}
...
// AOR Label 89
if(sizeof(PgHdr) / __builtin_offsetof(PgHdr, pDirty) != sizeof(PgHdr) - __builtin_offsetof(PgHdr, pDirty))  {
asm volatile (""::: "memory");
	report_label_coverage("AOR", 89);
}
// AOR Label 90
if(sizeof(PgHdr) + __builtin_offsetof(PgHdr, pDirty) != sizeof(PgHdr) - __builtin_offsetof(PgHdr, pDirty))  {
asm volatile (""::: "memory");
	report_label_coverage("AOR", 90);
}
// AOR Label 91
if(sizeof(PgHdr) * __builtin_offsetof(PgHdr, pDirty) != sizeof(PgHdr) - __builtin_offsetof(PgHdr, pDirty))  {
asm volatile (""::: "memory");
	report_label_coverage("AOR", 91);
}
...
// COR Label 121
if(((p->flags & 2) || (p->flags & 8)) != ((p->flags & 2) && (p->flags & 8)))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 121);
}

// MCC Label 122
if(( p->flags & 2 ) && ( p->flags & 8 ) ) {
asm volatile (""::: "memory");
...
// COR Label 143
if((pgno == 0 || pCache->nRefSum) != (pgno == 0 && pCache->nRefSum))  {
asm volatile (""::: "memory");
...
// ROR Label 155
if((pA->pgno <= pB->pgno) != (pA->pgno < pB->pgno))  {
asm volatile (""::: "memory");
...
for(; ALWAYS(i<N_SORT_BUCKET-1); i++){
...
for(; pDirty; pDirty=pDirty->pDirtyNext) { nDirty++;
};
// ABS Label 203
if(pDirty < 0)  {
asm volatile (""::: "memory");
