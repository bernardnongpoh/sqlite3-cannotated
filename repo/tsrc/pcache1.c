...
// ROR Label 13
if(((pcache1_g).nInitPage != 0) != ((pcache1_g).nInitPage == 0))  {
asm volatile (""::: "memory");
...
// ROR Label 15
if((pCache->nMax <= 3) != (pCache->nMax < 3))  {
asm volatile (""::: "memory");
...
// AOR Label 23
if(pCache->szAlloc - (i64)pCache->nMax != pCache->szAlloc * (i64)pCache->nMax)  {
asm volatile (""::: "memory");
...
if(pCache->szAlloc / (i64)pCache->nMax != pCache->szAlloc * (i64)pCache->nMax)  {
asm volatile (""::: "memory");
	report_label_coverage("AOR", 25);
}

// ROR Label 26
if((szBulk <= pCache->szAlloc * (i64)pCache->nMax) != (szBulk > pCache->szAlloc * (i64)pCache->nMax))  {
asm volatile (""::: "memory");
...
// ROR Label 31
if((pCache->pFree == 0) != (pCache->pFree != 0))  {
asm volatile (""::: "memory");
...
// ROR Label 34
if((nByte < (pcache1_g).szSlot) != (nByte <= (pcache1_g).szSlot))  {
asm volatile (""::: "memory");
...
// COR Label 71
if(((pcache1_g).nSlot || (pCache->szPage + pCache->szExtra) <= (pcache1_g).szSlot) != ((pcache1_g).nSlot && (pCache->szPage + pCache->szExtra) <= (pcache1_g).szSlot))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 71);
}

// ROR Label 72
if(((pCache->szPage + pCache->szExtra) < (pcache1_g).szSlot) != ((pCache->szPage + pCache->szExtra) <= (pcache1_g).szSlot))  {
asm volatile (""::: "memory");
...
// AOR Label 101
if(pPage->iKey - nNew != pPage->iKey % nNew)  {
asm volatile (""::: "memory");
...
// ROR Label 108
if(((*pp) == pPage) != ((*pp) != pPage))  {
asm volatile (""::: "memory");
	report_label_coverage("ROR", 108);
}
for(; (*pp)!=pPage; pp=&(*pp)->pNext) {;
};
// ABS Label 109
if((*pp) < 0)  {
asm volatile (""::: "memory");
...
// COR Label 119
if((pCache->nPage == 0 || pCache->pBulk) != (pCache->nPage == 0 && pCache->pBulk))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 119);
}

// ROR Label 120
if((pCache->nPage != 0) != (pCache->nPage == 0))  {
asm volatile (""::: "memory");
...
// AOR Label 129
if(pCache->iMaxKey / iLimit != pCache->iMaxKey - iLimit)  {
asm volatile (""::: "memory");
...
// ROR Label 132
if((pCache->iMaxKey - iLimit <= pCache->nHash) != (pCache->iMaxKey - iLimit < pCache->nHash))  {
asm volatile (""::: "memory");
...
// ROR Label 137
if((pPage->iKey < iLimit) != (pPage->iKey >= iLimit))  {
asm volatile (""::: "memory");
...
// COR Label 151
if(((pcache1_g).separateCache && sqlite3Config.nPage != 0 || sqlite3Config.pPage == 0) != ((pcache1_g).separateCache && sqlite3Config.nPage != 0 && sqlite3Config.pPage == 0))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 151);
}
// COR Label 152
if(((pcache1_g).separateCache || sqlite3Config.nPage != 0) != ((pcache1_g).separateCache && sqlite3Config.nPage != 0))  {
asm volatile (""::: "memory");
...
// ROR Label 166
if((pGroup->lru.isAnchor != 0) != (pGroup->lru.isAnchor == 0))  {
asm volatile (""::: "memory");
...
// ROR Label 169
if((pCache->nHash != 0) != (pCache->nHash == 0))  {
asm volatile (""::: "memory");
...
// ROR Label 180
if((pCache->nPage < pCache->nHash) != (pCache->nPage >= pCache->nHash))  {
asm volatile (""::: "memory");
...
// ROR Label 188
if((pOther->szAlloc == pCache->szAlloc) != (pOther->szAlloc != pCache->szAlloc))  {
asm volatile (""::: "memory");
...
// AOR Label 193
if(iKey - pCache->nHash != iKey % pCache->nHash)  {
asm volatile (""::: "memory");
...
// ROR Label 198
if((iKey <= pCache->iMaxKey) != (iKey > pCache->iMaxKey))  {
asm volatile (""::: "memory");
...
// COR Label 205
if((pPage || pPage->iKey != iKey) != (pPage && pPage->iKey != iKey))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 205);
}

// ROR Label 206
if((pPage->iKey == iKey) != (pPage->iKey != iKey))  {
asm volatile (""::: "memory");
	report_label_coverage("ROR", 206);
}

// MCC Label 207
if(pPage && pPage->iKey != iKey ) {
asm volatile (""::: "memory");
...
// ABS Label 226
if(pGroup->nMaxPage < 0)  {
asm volatile (""::: "memory");
	report_label_coverage("ABS", 226);
}
// ABS Label 227
if(pGroup->nPurgeable < 0)  {
asm volatile (""::: "memory");
...
// COR Label 229
if((reuseUnlikely && pGroup->nPurgeable > pGroup->nMaxPage) != (reuseUnlikely || pGroup->nPurgeable > pGroup->nMaxPage))  {
asm volatile (""::: "memory");
	report_label_coverage("COR", 229);
}

// ROR Label 230
if((pGroup->nPurgeable <= pGroup->nMaxPage) != (pGroup->nPurgeable > pGroup->nMaxPage))  {
asm volatile (""::: "memory");
...
// ROR Label 246
if((iNew <= pCache->iMaxKey) != (iNew > pCache->iMaxKey))  {
asm volatile (""::: "memory");
...
// ROR Label 251
if((iLimit < pCache->iMaxKey) != (iLimit <= pCache->iMaxKey))  {
asm volatile (""::: "memory");
